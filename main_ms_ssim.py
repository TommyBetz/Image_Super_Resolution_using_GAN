...
def train():
    ...
    ssim = tf.image.ssim_multiscale(net_g.outputs, t_target_image, max_val = 1.0)
    mean_ssim = tf.reduce_mean(ssim)
    mse_loss = tl.cost.mean_squared_error(net_g.outputs, t_target_image, is_mean=True) + (1 - mean_ssim)
